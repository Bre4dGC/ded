...
Errno read_entire_file(const char *file_path, String_Builder *strb)
{
    Errno result = 0;
    FILE *f = NULL;
    ...
    if (strb->capacity < size)
    {
        strb->capacity = size;
        strb->items = realloc(strb->items, strb->capacity * sizeof(*strb->items));
        assert(strb->items != NULL && "Buy more RAM lol");
    }

    fread(strb->items, size, 1, f);
    if (ferror(f))
        return_defer(errno);
    strb->count = size;
...
Errno type_of_file(const char *file_path, File_Type *ftype)
{
#ifdef _WIN32
